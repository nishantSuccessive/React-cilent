...
export class TraineeList extends React.Component {
  ...
  editHandleSubmit = (name, email) => {
    this.setState({
      open: false, editTrainee: false, user: '',
    });
...
deleteHandleCancel = () => {
  ...
  handleChangePage = (e, pages) => {
    ...
    callApi('get', `trainee?limit=${newlimit}&skip=${newskip}`, {}).then((res) => {
      if (res.data.data.count !== 0) {
        this.setState(
          { dataList: res.data.data.records, loading: false, totalData: res.data.data.count },
        );
      } else {
        this.setState(
          {
  ...
  render() {
    const {
      open, fullWidth, maxWidth, order, orderBy, rowsPerPage, page, editTrainee, deleteTrainee,
      user, error, dataList, loading, totalData,
    } = this.state;
